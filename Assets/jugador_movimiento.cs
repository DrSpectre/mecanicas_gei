...
public class JugadorMovimiento : MonoBehaviour
{
    ...
    /*void FixedUpdate(){
        Vector2 joystick = movimiento.ReadValue<Vector2>();

        // Vector3 dirrecion = transform.forward * joystick.y;
        
        

        rigid_body.MovePosition(transform.position + ((dirrecion * joystick.magnitude) * _velocidad_por_fotograma));

        // Seccion para el tanque y rotacion
        float voltear = joystick.x * velocidad_rotacion;

        Quaternion rotacion = Quaternion.Euler(0f, voltear, 0f);

        rigid_body.MoveRotation(rigid_body.rotation * rotacion);
